...
> 2026-07-13 기준 최신 코드(main, `403fdbd`)를 근거로 작성.
> 팀 동료들에게 이 프로젝트를 소개하기 위한 문서로, "이게 뭔지, 왜 필요한지, 어떻게 동작하는지, 다른 도구와 뭐가 다른지"에 전부 답할 수 있는 것을 목표로 한다.
...
| `webview_wait_for` | selector/JS 조건 대기 | 폴링 간격 50ms에서 1.5배씩 증가, 최대 200ms (지수 백오프). 기본 타임아웃 10초 |
...
| 대기 | `waitFor` | selector 출현/`gone`(소멸)/text/role/url — 100ms 폴링, 기본 5초 |
| | `sleep` | 고정 대기 |
