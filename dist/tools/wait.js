...
export const definition = {
    ...
    description: 'CSS selector가 나타나거나(selector), 사라지거나(gone), role 요소가 보이거나(role), JS 조건이 만족될 때까지(expression) 대기합니다. selector/gone/role/expression 중 하나 필수. (webview_flow의 waitFor와 인자 스펙 통일)',
    inputSchema: {
        type: 'object',
        properties: {
...
export async function handler(args) {
    try {
        ...
        if (!check) {
            return { isError: true, content: [{ type: 'text', text: 'selector / gone / role / expression 중 하나는 필수입니다.' }] };
        }
        ...
        while (true) {
            ...
            if (remaining <= 0) {
                return { isError: true, content: [{ type: 'text', text: `시간 초과 (${timeout}ms): ${check.goal}` }] };
            }
        ...
        return false;
            ...
            const result = (await cdp.send('Runtime.evaluate', { expression: inPage, awaitPromise: true, returnByValue: true }));
            if (result.result.value) {
                return { content: [{ type: 'text', text: `조건 충족 — ${check.label}` }] };
            }
        }
    }
    catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
        return { isError: true, content: [{ type: 'text', text: `대기 실패: ${msg}` }] };
    }
}
