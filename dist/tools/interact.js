...
async function findAndClick(cdp, selector, text, clearValue = false) {
    const script = buildFindScript(selector, text, clearValue);
    if (!script) {
        return { isError: true, content: [{ type: 'text', text: 'selector 또는 text 중 하나는 필수입니다.' }] };
    }
    ...
    if (coords.error === 'not_found') {
        ...
        return { isError: true, content: [{ type: 'text', text: `요소를 찾을 수 없습니다.${hint}` }] };
    }
